...
def _generate_blocks(problem: dict, cls: str, target: str,
    ...
    prompt = (
        ...
        f"after removal. Keep a reference BEFORE removing it - but ONLY to "
        f"something that LEAVES the object, like a node that was removed. "
        f"Never save a reference to a collection the object goes on using and "
        f"then read it again afterwards: that checks whether the method "
        f"replaced the collection or emptied it in place, which is a choice "
        f"the problem statement does not make, so it would fail a student who "
        f"chose the other one.\n"
