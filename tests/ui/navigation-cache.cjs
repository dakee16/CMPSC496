...
for(const graph of [nested,{nodes:[...nested.nodes].reverse(),edges:[...nested.edges].reverse()},diamond,
  ...
  for(const route of layout.routes){
    ...
    if(route.label)assert(route.label.x>=0&&route.label.x+route.label.w<=layout.w,"Clipped edge label");
...
(async()=>{
  ...
  console.log("PASS: graph ordering, orthogonal routing, label bounds, loop lanes, prototype-safe IDs, cache TTL/coalescing/account isolation/refresh/invalidation/errors.");
