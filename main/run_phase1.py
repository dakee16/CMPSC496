...
from .ollama_client import chat, DECOMPOSE_MODEL
...
# Decomposition runs once per problem at upload time and is reused by every
# student, so it buys the stronger model. See ollama_client for the measured
# 45% -> 80% difference this makes.
    ...
    for attempt in range(1, max_tries + 1):
        user_msg = (
            ...
            + (f"PREVIOUS ATTEMPT FAILED:\n{feedback}\n"
               ...
               "actual logic — re-split the problem at a different boundary, or "
               "fix the specific defect named above.\n\n" if feedback else "")
            + 'Return JSON only: {"subproblems": [{"prompt": "...", "reference": "..."}, ...]}'
        )
        # Temperature CLIMBS with each retry. At a flat 0.2 the model is close to
        # deterministic, so a retry regenerated byte-identical code and the
        # feedback changed nothing: measured on plus-one and subsets, five
        # attempts produced ONE distinct assembly between them — five paid calls,
        # one idea. Attempt 1 stays at 0.2 because the first answer should be the
        # model's best; only once it is known wrong is variety worth buying.
        temp = min(0.2 + 0.25 * (attempt - 1), 1.0)
        raw = chat(MODEL, CHUNK_DECOMPOSE_SYSTEM,
                   [{"role": "user", "content": user_msg}], temperature=temp, fmt="json")
        try:
            data = parse_json(raw)
